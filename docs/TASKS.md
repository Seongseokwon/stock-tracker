...
> **수익화 목표:** AdSense 승인 최단 경로 → A-0~A-2(브리핑 콘텐츠) → M-5(AdSense 신청)
...
| M-2c | 푸터에 링크 삽입 (index.html · login.html) | 2026-05-21 |
| M-3 | GA4 스크립트 삽입 (4개 HTML 파일) | 2026-05-21 |
| M-3b | GA4 측정 ID 교체 (`G-KMCM3VCKN6` 적용) | 2026-05-22 |
...
## 1순위 — AI 종목 브리핑 (콘텐츠 풍부화 · AdSense 핵심)
...
| ID | 타스크 | 상태 |
|----|--------|------|
| A-0 | 슬롯 규칙 정의 (ET 기준 `pre_open`·`midday`·`post_close`) | ✅ 2026-05-22 |
| A-1 | `GET /api/briefing?symbol=&slot=auto` — 시세·뉴스 데이터 수집 | ✅ 2026-05-22 |
| A-2 | 규칙 기반 요약 MVP (LLM 없음 — 주가 변동·뉴스 헤드라인 조합) | ✅ 2026-05-22 |
| A-4 | 모달 상단 「브리핑」 UI 섹션 | ✅ 2026-05-22 |
| A-5 | 슬롯·거래일 단위 캐시 (Railway DB `briefings` 테이블) | ✅ 2026-05-22 |
| A-6 | 장중 12시 전 안내 문구 | ✅ 2026-05-22 (slot=null 시 notice 반환) |
| A-7 | 면책 문구 연동 (M-2 완료) | ✅ 2026-05-22 (브리핑 본문 하단 포함) |
| A-8 | (선택) KR 뉴스 연동 후 브리핑 보강 | ⬜ K-3 이후 |

---

## 2순위 — AdSense 신청 및 슬롯 삽입

> 1순위(브리핑) 완료 후 신청. 콘텐츠·법적 고지·Analytics 3가지 갖춰야 합격률 높음.
...
> AdSense 수익이 LLM API 비용을 커버한 후 전환.
