...
| ID | 타스크 | 상태 |
|----|--------|------|
| M-2a | 면책고지 페이지 (`/disclaimer.html`) — 투자 정보 아님, 데이터 정확성 무보증 | ⬜ |
| M-2b | 개인정보처리방침 페이지 (`/privacy.html`) — 쿠키·세션·수집 항목 명시 | ⬜ |
| M-2c | 푸터에 링크 삽입 (면책·개인정보·이용약관) | ⬜ |
...
| ID | 타스크 | 상태 |
|----|--------|------|
| M-3 | GA4 스크립트 삽입 (`index.html`, `login.html`) | ⬜ |
